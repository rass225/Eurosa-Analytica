...
public struct Line: View {
    ...
    var gradient: GradientColor = GradientColor(start: Eurosa.Colors.blue1, end: Eurosa.Colors.blue1)
    var index:Int = 0
    ...
    public var body: some View {
        ZStack {
            if(self.showFull && self.showBackground){
                self.closedPath
                    .fill(LinearGradient(gradient: Gradient(colors: [Eurosa.Colors.blue1.opacity(1), .white]), startPoint: .bottom, endPoint: .top))
                    .rotationEffect(.degrees(180), anchor: .center)
                    .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
                    .transition(.opacity)
                    .animation(.easeIn, value: 1.6)
            }
            self.path
                ...
                .rotationEffect(.degrees(180), anchor: .center)
                .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
                .animation(.easeOut.delay(Double(self.index)*0.4), value: 1.2)
                .onAppear {
                    self.showFull = true
            }
            .onDisappear {
